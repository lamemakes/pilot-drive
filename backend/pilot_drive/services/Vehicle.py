...
class Vehicle(AbstractService):
    def __init__(self, master_event_queue: MasterEventQueue, service_type: EventType, obd_port: str = None):
        '''
    ...
    def __is_connected(self):
        if self.__connection and self.__connection.status() in [obd.OBDStatus.ELM_CONNECTED, obd.OBDStatus.OBD_CONNECTED, obd.OBDStatus.CAR_CONNECTED]:
    ...
    def __handle_connect(self):
        if self.__obd_port:
            if os.path.exists(self.__obd_port):
                ...
                raise InvalidPortException('Specified path: "' + self.__obd_port + '" does not exist!')
        else:
            ...
            for field in QUERIED_FIELDS:
                ...
                if obd.commands.has_name(field_command):
                    ...
                    if queries_made % field_interval == 0:
                        ...
                        if resp.value:
                            resp_tuple = resp.value.to_tuple()        # Convert to a tuple to get units & magnitude
                            value = {'quantity': resp_tuple[0], 'units': resp_tuple[1][0][0], 'magnitude': resp_tuple[1][0][1]}   # pint converts tuples a little odd, as values come back as in the form of "(<quantity>, (('<unit>', <magnitude>),))" - lots of nesting.
                            if len(self.stats) == field_index:
                                self.stats.append({'name': field_name, 'value': value})
                            else:
                                self.stats[field_index] = {'name': field_name, 'value': value}
                    ...
                    raise InvalidQueryException('Specified OBD command: "' + field_name + '" is not valid!')
    ...
    def main(self):
        ...
        while True:
            ...
            if self.__is_connected():
                ...
                time.sleep(0.5)
                queries_made = 0 if queries_made == self.__max_interval else queries_made + 0.5
