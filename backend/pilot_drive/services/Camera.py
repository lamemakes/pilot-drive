...
class CameraManager(AbstractService):
    def __init__(self,
        ...
        super().__init__(master_event_queue=master_event_queue, service_type=service_type, logger=logger)

        try:
            import RPi.GPIO as GPIO # Import Raspberry Pi GPIO library
            import picamera
        except ModuleNotFoundError as err:
            raise ServiceExceptions.FailedToInstatiateCamera(f'Failed to find a required module, camera service will not be started: {err}')
        ...
        self.__GPIO.add_event_detect(btn_pin, GPIO.RISING, callback=self.show_camera, bouncetime=400)

        # Initialize the camera
        try:
            self.camera = picamera.PiCamera()
        except picamera.PiCameraError as err:
            raise ServiceExceptions.FailedToInstatiateCamera(f'Failed to open PiCamera: {err}')
